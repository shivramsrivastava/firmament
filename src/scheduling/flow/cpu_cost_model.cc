...
ArcCost_t CpuCostModel::GetPodGroupDRFArchCost(string pod_group_name) {
	...
	if(pod_grp_to_arch_cost != NULL) {
		...
		if(arch_cost_ptr != NULL) {
			...
			arch_cost = INVALID_ARCH_COST;//invalid arch cost ...TBD do we need to do somthing like assert?
		}
	} else {
		arch_cost = INVALID_ARCH_COST;//invalid arch cost ...TBD do we need to assert?
	}
